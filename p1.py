...
def p1b(is_test=False, test_file_name="1t.txt"):
    def nof_occurences(array, n):
        filter = [a == n for a in array]
        return len(array[filter])
    ...
    pattern = r"^(\d+)\s+(\d+)$"
    pairs_orig = [re.match(pattern, l).group(1, 2) for l in lines]
    left = [int(p[0]) for p in pairs_orig]
    right = np.array([int(p[1]) for p in pairs_orig])
    f = partial(nof_occurences, right)
    distance = sum([n*f(n) for n in left])
    return distance